...
{
    // interpret -noDRS as -DRS=0 (and -noDRS=0 as -DRS=1) as long as -DRS not set
    if (name.find("-no") == 0)
    {
        std::string positive("-");
...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        //  interpret --DRS as -DRS (as long as both are not set)
        if (name.find("--") == 0)
        {
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No Denarius.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override Denarius.conf
        string strKey = string("-") + it->string_key;
        if (mapSettingsRet.count(strKey) == 0)
        {
            mapSettingsRet[strKey] = it->value[0];
            // interpret noDRS=1 as DRS=0 (and noDRS=0 as DRS=1) as long as DRS not set)
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Denarius.pid"));
