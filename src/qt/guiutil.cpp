...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    //
    //    Cannot handle this later, because Denarius:// will cause Qt to see the part after // as host,
    //    which will lowercase it (and thus invalidate the address).
    if(uri.startsWith("Denarius://"))
    {
        uri.replace(0, 11, "Denarius:");
    }
                                 ...
                                 QString *selectedSuffixOut)
{
    ...
    /* Extract first suffix from filter pattern "Description (*.DRS)" or "Description (*.DRS *.bar ...) */
    QRegExp filter_re(".* \\(\\*\\.(.*)[ \\)]");
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Denarius.desktop file to the autostart directory:
